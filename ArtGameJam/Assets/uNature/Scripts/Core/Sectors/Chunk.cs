...
namespace uNature.Core.Sectors
{
    ...
    public class Chunk : MonoBehaviour
    {
        ...
        {
            Vector2 _size = new Vector2(size.x + offset, size.y + offset);

            Vector2 _minPoint = new Vector2(minPoint.x - _size.x, minPoint.y - _size.y);
            Vector2 _maxPoint = new Vector2(minPoint.x + _size.x, minPoint.y + _size.y);

            return point.x >= _minPoint.x && point.z >= _minPoint.y && point.x <= _maxPoint.x && point.z <= _maxPoint.y;
        }
